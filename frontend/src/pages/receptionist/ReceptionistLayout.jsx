...
const ReceptionistLayout = ({ children }) => {
    ...
    const navItems = [
        { path: '/receptionist/dashboard', icon: LayoutDashboard, label: 'Overview' },
        { path: '/receptionist/register', icon: UserPlus, label: 'Register Patient' },
        { path: '/receptionist/search', icon: Search, label: 'Search Patients' },
        { path: '/receptionist/bookings', icon: Calendar, label: 'Manage Bookings' },
    ];
                    ...
                    <div className="p-4 mt-auto">
                        <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 flex items-center gap-3 hover:border-slate-300 transition-premium cursor-pointer group mb-2">
                            ...
                            <div className="min-w-0 flex-1">
                                <p className="font-bold text-slate-900 text-sm truncate">{user?.name || 'Receptionist'}</p>
                                <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider truncate">{user?.email || 'Staff'}</p>
                            </div>
                ...
                <div className="flex-1 flex flex-col h-screen overflow-hidden bg-[#F8FAFC]">
                    ...
                    <header className="hidden lg:flex items-center justify-between px-8 py-5 bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-20">
                        ...
                        <div className="flex items-center relative group w-96">
                            <SearchIcon size={16} className="text-slate-400 absolute left-3 group-focus-within:text-blue-500 transition-colors" strokeWidth={2.5} />
                            <input
                                type="text"
                                ...
                                {notificationsOpen && (
                                    <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-hover border border-slate-100 z-50 overflow-hidden animate-fadeIn">
                                        ...
                                        <div className="max-h-96 overflow-y-auto">
                                            {[
                                                { id: 1, title: 'New Patient Registered', desc: 'John Doe has been registered successfully', time: '5m ago', type: 'success', unread: true },
                                                { id: 2, title: 'Appointment Request', desc: 'New booking request from Sarah Smith', time: '1h ago', type: 'alert', unread: true },
                                                { id: 3, title: 'Booking Cancelled', desc: 'Patient Michael Brown cancelled appointment', time: '3h ago', type: 'doc', unread: false }
                                            ].map(n => (
